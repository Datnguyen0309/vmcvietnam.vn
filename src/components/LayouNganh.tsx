...
export const LayoutNganh = ({ category }: LayoutNganhProps) => {
  ...
  const { data: allCourses, isLoading } = useQuery(["allCourses", category], () => fetchAllCourses(category), {
  ...
  const filterByKeyword = (list: { name: string }[], keyword: string) => {
    ...
    return list.filter((item) => {
      ...
      return normalizedKeywordWords.every((word) => normalizedNameWords.some((nameWord) => nameWord.includes(word)))
    })
  }
    ...
    <div className="container max-w-7xl mx-auto px-4 py-20">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
        <div className="flex items-center gap-2 text-[18px] text-gray-600">
          ...
          <span>/</span>
          <Link href="/khoa-hoc"  className="font-[900] text-[#222]">
              ...
              <div className="flex whitespace-nowrap justify-center items-center gap-2 px-2 min-w-fit w-full">
                ...
                {generatePageNumbers().map((page, idx) => (
                  <button
                    ...
                    className={`w-10 h-10 flex-shrink-0 flex items-center justify-center rounded border transition-all ${
                      currentPage === page
