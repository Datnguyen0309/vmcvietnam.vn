...
export default function EventTabs({ eventSections }: EventTabsProps) {
  const [activeTab, setActiveTab] = useState<"all" | "upcoming">("all") // Xóa "ongoing"
  const [dateRange, setDateRange] = useState<{ startDate?: Date; endDate?: Date }>({})
  ...
  return (
    <div>
      <h2 className="text-4xl font-bold text-center text-[#4A306D] mb-8">SỰ KIỆN NỔI BẬT</h2>
      <div className="flex justify-center gap-4 mb-8">
        {["all", "upcoming"].map((tab) => (  // Xóa "ongoing" khỏi đây
          <button
            key={tab}
            className={`px-6 py-2 rounded-full transition-colors ${activeTab === tab ? "bg-red-400 text-white" : "bg-white text-[#4A306D] hover:bg-gray-100"}`}
            onClick={() => setActiveTab(tab as any)}
          >
            {{
              all: "Tất Cả",
